...
**Decision:** Default `PROVIDER_MODE=local` and `CLOUD_AI_ENABLED=false`. Local providers process data in-process. Any future cloud provider may only be invoked through the Data Boundary Gateway after policy ALLOW. OpenAI is optional and not a P1 dependency.
